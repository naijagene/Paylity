...
class ProductCatalogService
{
     ...
     * @return array<string, mixed>
     */
    public function catalogResponse(?string $category = null): array
    {
        ...
        if ($category === null || $category === 'data') {
            $response['data_services'] = $this->mapDataServices();
        }
     ...
     * @return list<array<string, mixed>>
     */
    private function mapDataServices(): array
    {
        return ProviderService::query()
            ->with(['variations' => fn ($query) => $query->where('is_active', true)->orderBy('name')])
            ->where('provider', self::PROVIDER_VTPASS)
